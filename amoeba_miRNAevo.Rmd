...
palette2 <- c("#AE2012","#CA6B02","#FFCF3B","#B5E48C","#76C893","#34A0A4","#1E6091")
palette3 <- c("#AE2012","#FFCF3B","#E8EDED","#76C893","#34A0A4","#1E6091")
```
...
all_LP_mir$rna_type[all_LP_mir$rna_type=="complementary_rRNA"|
            ...
            all_LP_mir$rna_type=="complementary_ncRNA"] <- "intergenicRNA"
all_LP_mir$rna_type[all_LP_mir$rna_type=="ncRNA"] <- "other_ncRNA"
all_LP_mir <- all_LP_mir %>% group_by(readlength, rna_type, species) %>% summarise(reads = sum(reads))
all_LP_mir$rna_type <- factor(all_LP_mir$rna_type, levels = c("miRNA","intergenicRNA","transposon","tRNA","snRNA","mRNA","complementary_mRNA","rRNA","other_ncRNA"))
grouped_all_LP_mir <- all_LP_mir %>% group_by(rna_type ,species ) %>% summarise(reads = sum(reads))
all_LP_mir$reads <- all_LP_mir$reads/1E6


LP_mir_sizesel <- all_LP_mir[all_LP_mir$readlength>=20 & all_LP_mir$readlength<=24, ] 
LP_mir_sizesel$rna_type[LP_mir_sizesel$rna_type=="complementary_mRNA"]<-"intergenicRNA"
LP_mir_sizesel %>% group_by(rna_type, species) %>%
  summarise(sum(reads)) -> LP_mir_sizesel

plotdat <- LP_mir_sizesel[
  #LP_mir_sizesel$rna_type!="snRNA"&LP_mir_sizesel$rna_type!="tRNA"&LP_mir_sizesel$rna_type!="other_ncRNA"&
  LP_mir_sizesel$rna_type!="rRNA"&LP_mir_sizesel$rna_type!="mRNA"&
  LP_mir_sizesel$species!="dfir_new",]
plotdat$species <- factor(plotdat$species, levels = rev(species_list))
a <- ggplot(plotdat, aes(x=species,y=plotdat$`sum(reads)`, fill = rna_type))+
  geom_bar(stat = "identity", position  = "fill", color="#222222")+
  coord_flip()+
  theme(legend.position = "right", 
        ...
        axis.title.x = element_text(size = 12))+
  scale_fill_manual(values = palette3 , guide = guide_legend(reverse = FALSE), name = "RNA type:")+
  scale_x_discrete(labels = labels, name = element_blank())+
  scale_y_continuous(name = "Fraction of 20-24nt ncRNA reads")
a
ggsave("plots/miRNA_expression.tiff", a, device = "tiff", dpi = 200, scale = 2, width = 3, height = 2.5)
```


```{r Length profiles with annotations,  fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
b <- ggplot(all_LP_mir[all_LP_mir$rna_type!="other_ncRNA"&all_LP_mir$rna_type!="snRNA"&all_LP_mir$rna_type!="rRNA"&all_LP_mir$species!="dfir",],
...
c <- ggplot(all_LP_mir[all_LP_mir$rna_type=="miRNA"&all_LP_mir$species!="dfir",],
  aes(readlength,reads, fill = rna_type))+ 
  facet_wrap(~species, scales = "free", ncol = 3, labeller = labeller(species = labels))+
  geom_bar(stat = "identity", color="#333333", size = 0.6, width = 0.8)+
  labs(y = "million mapped reads", x="read length",fill="RNA type")+
...
#                 rowMeans(cbind(fcounts1$counts/(fcounts1$stat[1,2]+fcounts1$stat[2,2]),
#                                fcounts2$counts/(fcounts2$stat[1,2]+fcounts2$stat[2,2]),
#                                fcounts3$counts/(fcounts3$stat[1,2]+fcounts3$stat[2,2])))*1e6,
#                 species)
...
colnames(miRNA_characteristics) <- c("miRNA.ID","miRNA_sequence","miRNA_counts","miRNA_RPM","precision","genomic_context","genomic_location","miRNA_family","folding energy (kcal/mol)","pre-miRNA size (nt)")


miRNA_characteristics$miRNA_RPM <- round(miRNA_characteristics$miRNA_RPM, digits = 0)
...
#tiff("plots/discoideum_firmibasis_circos.png",width = 3600, height = 3450 , res = 420)
circos.par(start.degree = 90)
circos.genomicInitialize(combined)
circos.track(ylim = c(0, 1), 
             bg.col = c(rep("white",6),brewer.pal(6,"Dark2"),rep("#808080",6)), 
             bg.border = "#000000", bg.lwd = 2, track.height = 0.05)
for (i in 1:6) {
  circos.genomicLink(links_df[links_df[,1]==discoideum$name[i],1:3],
                     links_df[links_df[,1]==discoideum$name[i],4:6],
                     col = paste0(brewer.pal(6, "Dark2")[i],"40"))
}
dev.off()
...
micounts <- as.data.frame(read.table("data_in/miRNA_counts.txt", sep = "\t", header = T))
sicounts <- as.data.frame(read.table("data_in/siRNA_counts.txt", sep = "\t", header = T))
counts <- rbind(micounts,sicounts)
colnames(counts) <- gsub("mapped.", "", colnames(counts))
...
meta_data_rna <- do.call("rbind",sapply(colnames(counts), function(x){strsplit(x, "\\.|_")})) %>%
	data.frame() %>%
	select(StrainID = X1, Genotype = X2, Extra = X3, Time = X4, Rep = X5) %>%
	mutate(Time = str_pad(gsub("h", "", Time), 2, pad="0")) %>%
	mutate(sample_id = rownames(.)) %>%
	arrange(StrainID, Time, Rep)
...
dds <- DESeq(dds)
res <- results(dds, contrast = c("Genotype","drnB","wt"))
...
```{r drnBKO growthcurves, fig.height=3.5, fig.width=7.5, message=FALSE, warning=FALSE}
kleb_counts <- data.frame(t(read.delim("data_in/growthcurve_counts/drnb_klebsiella.txt", header = F, row.names = 1)))
kleb_growth <- pivot_longer(kleb_counts, -c(time, techrep))
kleb_growth %>% mutate(time = as.numeric(time),
                       value = as.numeric(value)*1e4) -> kleb_growth
...
all_growth <- rbind(axenic_growth,kleb_growth)
all_growth %>% separate(name, c("strain","growth","biorep"), "_", remove = F) -> all_growth

all_growth$ID <- factor(paste(all_growth$strain,all_growth$growth,sep = "_"),
                        levels = c("wt_klebsiella","drnb_klebsiella","wt_axenic","drnb_axenic"), 
                        labels = c("wt klebsiella","drnB- klebsiella","wt axenic","drnB- axenic"))


a <- ggplot(all_growth[all_growth$growth=="axenic",], aes(time, value, color = ID, shape = biorep))+
  geom_point(size=2)+
  geom_smooth(method = "lm", se = F, linetype = "dashed", size = 0.5)+
  scale_y_log10()+
  scale_color_manual(values = c("#1F78B4","#33A02C"))+
  labs(x = "Time (h)", y = "Number of cells / ml", color = "strain + growth condition")+
...
doubling_times <- data.frame(cbind(doubling_time = round(1/coef(lmList(log2(value)~time|name , data = all_growth, na.action=na.exclude)),digits = 2)[,2],
                        strain = rep(c("drnb","wt"), each = 7),
                        growth = rep(c(rep("axenic" ,4),rep("klebsiella",3)),2))
)
print(doubling_times)
doubling_times$doubling_time <- round(as.numeric(doubling_times$doubling_time), digits = 2)
doubling_times$ID <- factor(paste(doubling_times$strain, doubling_times$growth, sep = "_"),
                            levels = c("wt_klebsiella","drnb_klebsiella","wt_axenic","drnb_axenic"))
...
ttests <- compare_means(doubling_time ~ ID, data = doubling_times, method = "t.test")
plot_labels <- rep("",4)
plot_labels[c(1,3)] <- paste0("p=",round(ttests$p,digits = 3))[c(1,6)]


b <- ggplot(plotdat[3:4,], aes(ID,mean_doubling_time))+
  #geom_bar(stat = "identity", color = "black", fill = c("#A6CEE3","#1F78B4","#B2DF8A","#33A02C"))+
  geom_bar(stat = "identity", color = "black", fill = c("#1F78B4","#33A02C"))+
  geom_text(aes(label = round(mean_doubling_time, digits = 1)), vjust = 1.5)+
  geom_errorbar(aes(ymin=mean_doubling_time,ymax=mean_doubling_time+std), width = .2)+
  geom_text(aes(label = plot_labels[3:4], y=33), hjust = 0.1)+
  labs(y = "Doubling time (h)", x = "")+
  #scale_x_discrete(labels = c("wt klebsiella","drnB- klebsiella","wt axenic","drnB- axenic"))+
  scale_x_discrete(labels = c("wt axenic","drnB- axenic"))+
...
arr_plot <- ggarrange(a,b, widths = c(3,1))
